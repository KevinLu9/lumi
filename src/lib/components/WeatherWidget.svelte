<script lang="ts">
  ...
  function iconFor(code: number): Component {
    ...
    // Snow / sleet / freezing.
    if (
      [179, 182, 185, 227, 230, 281, 284, 311, 314, 317, 320, 323, 326, 329, 332, 335, 338,
       350, 362, 365, 368, 371, 374, 377].includes(code)
    )
      return CloudSnow
  ...
  <div class="days">
    {#each days as d}
      ...
      <div class="day" title={d.desc}>
        ...
        <div class="temps"><span class="max">{d.max}°</span><span class="min">{d.min}°</span></div>
...
<style>
  ...
  .temps { display: flex; gap: 5px; align-items: baseline; font-size: 13px; }
  .temps .max { color: var(--text); font-weight: 600; }
  .temps .min { color: var(--text-dim); }
  .rain {
    display: inline-flex;
    align-items: center;
    gap: 2px;
  ...
  .rain.dry { color: var(--text-dim); opacity: 0.6; }
</style>
